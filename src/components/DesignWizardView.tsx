...
export function DesignWizardView({ aiEngine }: { aiEngine: 'gemini' | 'zai' }) {
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-gray-800">1. ¿Qué vas a diseñar hoy?</h3>
        <p className="text-gray-500">Elige el tipo de producto. Esto ayudará a la IA a definir las proporciones y la vibra del arte.</p>
      </div>
        ...
        {PRODUCT_CATEGORIES.map(category => (
          <div key={category.id} className="space-y-3">
            <div className="flex items-center gap-2 border-b pb-2">
              ...
              <span className="text-xs text-gray-400 ml-2 hidden md:inline">- {category.description}</span>
            </div>
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="text-center mb-8">
          <h3 className="text-2xl font-bold text-gray-800 flex items-center justify-center gap-2">
            2. Dirección de Arte <span className="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full font-normal">Para {selectedProduct.name} {selectedProduct.icon}</span>
          </h3>
          <p className="text-gray-500">Hemos filtrado los estilos que estadísticamente mejor convierten para este producto.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {recommended.map(style => (
            <Card 
              key={style.id} 
              className={`cursor-pointer transition-all border-2 ${selectedStyle?.id === style.id ? 'border-indigo-500 bg-indigo-50/30 ring-2 ring-indigo-200' : 'border-gray-200 hover:border-indigo-300'}`}
            ...
            >
              <CardContent className="p-5 relative h-full flex flex-col">
                ...
                <div className="flex items-start justify-between mb-2">
                  <h4 className="font-bold text-gray-800 pr-6">{style.name}</h4>
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button className="text-gray-400 hover:text-indigo-500 z-10" onClick={(e) => e.stopPropagation()}>
                ...
                <p className="text-xs text-gray-600 mb-4 flex-1">{style.description}</p>
                
                <div className="mt-auto bg-gray-50 p-2 rounded-lg border border-gray-100">
                  <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider block mb-1">Nichos Recomendados:</span>
                  <span className="text-xs text-indigo-700 font-medium">{style.recommendedNiches}</span>
                </div>
              </CardContent>
            </Card>
          ))}
  ...
  const renderStep3 = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
        <div className="text-center mb-8">
          <h3 className="text-2xl font-bold text-gray-800">3. Toque Final: Paleta y Nicho</h3>
          <p className="text-gray-500">Cierra tu idea para que la IA arme el Prompt Maestro.</p>
        </div>

        <div className="grid md:grid-cols-2 gap-8">
          {/* Left: Palettes */}
          <div className="space-y-4">
            <h4 className="font-semibold text-gray-700 flex items-center gap-2"><Paintbrush className="w-4 h-4" /> Paletas de Color para "{selectedStyle.name}"</h4>
            <div className="space-y-3">
              {selectedStyle.palettes.map(palette => (
                <div 
                  key={palette.name}
                  onClick={() => setSelectedPalette(palette)}
                  className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedPalette?.name === palette.name ? 'border-indigo-500 bg-indigo-50/50' : 'border-gray-200 hover:border-gray-300'}`}
                >
                  ...
                  <div className="flex gap-2 h-10 w-full rounded-lg overflow-hidden border border-gray-100 shadow-sm">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="font-semibold text-gray-700 text-sm">¿Cuál es el nicho o temática de tu diseño?</label>
              <Input 
              ...
              />
              <p className="text-xs text-gray-400">Te sugerimos: {selectedStyle.recommendedNiches}</p>
            </div>

            <Card className="bg-gray-50 border-gray-200 shadow-inner">
              <CardContent className="p-4 space-y-3">
                <h5 className="font-bold text-gray-700 text-sm border-b pb-2">Resumen de Instrucción</h5>
                <div className="text-sm space-y-2">
                  <p><span className="text-gray-500">Producto:</span> <span className="font-semibold">{selectedProduct?.name}</span></p>
                  <p><span className="text-gray-500">Estilo:</span> <span className="font-semibold">{selectedStyle?.name}</span></p>
                  <p><span className="text-gray-500">Paleta:</span> <span className="font-semibold">{selectedPalette?.name || '(Selecciona una)'}</span></p>
                  <p><span className="text-gray-500">Nicho:</span> <span className="font-semibold italic">{niche || '(Escribe tu nicho)'}</span></p>
                </div>
              </CardContent>
            </Card>
          </div>
  ...
  const renderStep4 = () => (
    <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500 max-w-3xl mx-auto">
      <div className="text-center mb-8">
        ...
        <h3 className="text-2xl font-bold text-gray-800">¡Tu Prompt Maestro está listo!</h3>
        <p className="text-gray-500">Copia este texto y úsalo en Midjourney, DALL-E 3 o Ideogram para obtener tu diseño perfecto.</p>
      </div>
      ...
      <Card className="border-green-200 bg-green-50/30">
        <CardContent className="p-6 relative">
          <textarea 
            readOnly
            className="w-full h-64 p-4 bg-white border border-gray-200 rounded-xl text-sm font-mono text-gray-700 focus:ring-green-500 focus:border-green-500"
      ...
      <Card className="mt-8 border-indigo-100 bg-white shadow-sm">
        ...
        <CardContent className="p-6 space-y-4">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700">Título del Prompt</label>
              <Input 
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700">Seleccionar Proyecto</label>
              <select 
          ...
          {selectedProjectId === 'new' && (
            <div className="space-y-2 pt-2">
              <label className="text-sm font-semibold text-gray-700">Nombre del Nuevo Proyecto</label>
              <Input 
    ...
    <div className="max-w-5xl mx-auto py-8">
      ...
      <div className="mb-10 max-w-md mx-auto">
        <div className="flex justify-between items-center relative">
          ...
          <div className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-indigo-500 transition-all duration-500 z-0" style={{ width: `${((step - 1) / 3) * 100}%` }}></div>
          ...
          {[1, 2, 3, 4].map(num => (
            <div key={num} className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${
              step >= num ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-gray-200 text-gray-400'
            }`}>
