    ...
    <div className="max-w-6xl mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Genera arte digital comercial de alta calidad directamente desde tus Prompts Maestros.</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="border-indigo-100 shadow-sm">
            <CardHeader className="bg-indigo-50/50 pb-4 border-b">
              <CardTitle className="text-lg flex items-center gap-2">Configuración de Generación</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-6">

              {/* Prompt Input */}
              <div className="space-y-3">
                <label className="text-sm font-bold text-gray-700">1. Ingresa tu Prompt Maestro</label>
                <textarea 
                  className="w-full h-40 p-4 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none text-sm font-mono"
              ...
              <div className="flex flex-col gap-3">
                <Button 
                  ...
                  className="w-full h-12 bg-indigo-600 hover:bg-indigo-700 text-md font-bold"
                >
                  {generating ? (
                ...
                <Button 
                  ...
                  className="w-full h-12 text-md font-bold text-gray-600 hover:text-gray-900 border-gray-300"
                >
        ...
        <div className="lg:col-span-7 flex flex-col h-full">
          <Card className="flex-1 border-gray-200 bg-gray-50 overflow-hidden flex flex-col relative min-h-[500px]">
            {currentImage ? (
              <>
                <div className="absolute top-4 right-4 z-10 flex gap-2">
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button variant="secondary" size="icon" className="bg-white/90 backdrop-blur-sm hover:bg-white text-indigo-600 border-indigo-200" onClick={() => handleRegenerateSpecific(currentImage.prompt)}>
                  ...
                  <Button variant="secondary" size="icon" className="bg-white/90 backdrop-blur-sm hover:bg-white" onClick={() => downloadImage(currentImage.url)}>
                    <Download className="w-5 h-5 text-gray-700" />
                  </Button>
                  <Button variant="secondary" size="icon" className="bg-white/80 backdrop-blur-sm hover:bg-white" onClick={() => window.open(currentImage.url, '_blank')}>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center h-full">
                {generating ? (
                  <div className="animate-pulse flex flex-col items-center">
                    ...
                    <p className="font-semibold text-gray-600">Creando arte con IA...</p>
                    <p className="text-xs text-gray-400 mt-2">Aplicando filtros de diseño plano y trazados</p>
                  </div>
                ) : (
                  <>
                    <ImageIcon className="w-20 h-20 text-gray-300 mb-4" />
                    <h3 className="text-xl font-medium text-gray-500">Lienzo en Blanco</h3>
          ...
          {history.length > 0 && (
            <div className="mt-4">
              <h4 className="text-sm font-semibold text-gray-500 mb-2">Generaciones Recientes</h4>
              <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                {history.map((img, idx) => (
                  <div 
                    key={img.id} 
                    className={`relative group shrink-0 w-20 h-20 rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${currentImage?.id === img.id ? 'border-indigo-500' : 'border-transparent hover:border-gray-300'}`}
